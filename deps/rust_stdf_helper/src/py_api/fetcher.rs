...
impl PyDataFetcher {
    ...
    pub fn get_site_list(&self) -> PyResult<Vec<i64>> {
        Ok(self.inner.get_site_list()?)
    }

    pub fn get_head_list(&self) -> PyResult<Vec<i64>> {
        Ok(self.inner.get_head_list()?)
    }
    ...
    pub fn get_test_info<'py>(
        ...
        let info = self
            .inner
            .get_test_info((test_num, test_name), file_id)?;
        ...
        if let Some(info) = info {
            ...
            dict.set_item("LLimit", info.llimit.unwrap_or(f64::NAN))?;
            dict.set_item("HLimit", info.hlimit.unwrap_or(f64::NAN))?;
            ...
            dict.set_item("LSpec", info.lspec.unwrap_or(f64::NAN))?;
            dict.set_item("HSpec", info.hspec.unwrap_or(f64::NAN))?;
    ...
    pub fn get_test_data_from_head_site<'py>(
        ...
        let fetched = self
            .inner
            .get_test_data_from_head_site((test_num, test_name), &heads, &sites, file_id)?;
        let dict = PyDict::new(py);
        match fetched {
            Some(data) => {
                ...
                if data.rec_header == 10 {
                    ...
                    let flags: numpy::ndarray::Array1<u8> =
                        data.flags.mapv(|v| v as u8);
