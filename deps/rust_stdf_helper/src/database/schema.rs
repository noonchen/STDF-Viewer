...
/// Tables a valid session must contain; keep in sync with `CREATE_TABLE_SQL`.
pub(crate) static REQUIRED_TABLES: [&str; 15] = [
    "File_List",
    "File_Info",
    "Wafer_Info",
    "Dut_Info",
...
/// Query indexes, in build order. `DataFetcher` creates any that are missing in
/// a background thread after the database is opened, one statement per
/// transaction so the build is resumable and can stop between indexes.
pub(crate) static INDEX_NAMES: [&str; 4] = ["ptrKey", "mprKey", "ftrKey", "dynKey"];

pub(crate) static CREATE_INDEX_STATEMENTS: [&str; 4] = [
    "CREATE INDEX IF NOT EXISTS ptrKey ON PTR_Data (TEST_ID, DUTIndex)",
    "CREATE INDEX IF NOT EXISTS mprKey ON MPR_Data (TEST_ID, DUTIndex)",
    "CREATE INDEX IF NOT EXISTS ftrKey ON FTR_Data (TEST_ID, DUTIndex)",
    "CREATE INDEX IF NOT EXISTS dynKey ON Dynamic_Limits (TEST_ID, DUTIndex)",
];

pub(crate) static FETCH_INDEX_EXISTS: &str =
    "SELECT count(*) FROM sqlite_master WHERE type='index' AND name=?";

/// Finish the bulk load. The database is handed over in WAL mode so query
/// readers (any SQLite build, any connection) can run while the fetcher builds
/// its indexes in the background.
