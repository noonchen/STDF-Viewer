...
class DatabaseFetcherCached:
    def __init__(self):
        self.connection = None
        self.cursor = None
        self.file_paths = []
        self.full_dut_cache = {}            # fid -> np.ndarray([1..N], dtype=uint32)
        self.head_site_cache = {}           # (fid, head, site_or_None) -> np.ndarray(row indices)
        self.test_info_cache = {}           # (fid, test_num, test_name) -> dict
        self.test_data_cache = {}           # (test_id, fid) -> dict with full arrays + valid_test_idx
    ...
    def closeDB(self):
        ...
        self.connection = None
        self.cursor = None
        self.full_dut_cache.clear()
        self.head_site_cache.clear()
        self.test_info_cache.clear()
    ...
    def getTestInfo(self, testTup: tuple, fileId) -> dict:
        '''fetch a test info of testID from file id, cached'''
        if self.cursor is None: raise RuntimeError("No database is connected")
        return self._get_test_info(testTup, fileId)
    
    
    def _build_caches(self):
        '''Build canonical DUT arrays and head/site row-index caches.'''
        self.full_dut_cache.clear()
        self.head_site_cache.clear()
        self.test_info_cache.clear()
        self.test_data_cache.clear()
        ...
        for fid in range(self.num_files):
            row = self.cursor.execute(
                "SELECT MAX(DUTIndex) FROM Dut_Info WHERE Fid=?", (fid,)
            ).fetchone()
            n = int(row[0]) if row and row[0] else 0
            self.full_dut_cache[fid] = np.arange(1, n + 1, dtype=np.uint32)

            site_lists = {}
            head_lists = {}
            for dut_index, head_num, site_num in self.cursor.execute(
                "SELECT DUTIndex, HEAD_NUM, SITE_NUM FROM Dut_Info WHERE Fid=? AND Supersede=0",
                (fid,),
            ):
                row_idx = int(dut_index) - 1
                head = int(head_num)
                site = int(site_num)
                site_lists.setdefault((fid, head, site), []).append(row_idx)
                head_lists.setdefault((fid, head, None), []).append(row_idx)

            for key, vals in site_lists.items():
                self.head_site_cache[key] = np.asarray(sorted(vals), dtype=np.intp)
            for key, vals in head_lists.items():
                self.head_site_cache[key] = np.asarray(sorted(vals), dtype=np.intp)

    def _get_test_info(self, testTup: tuple, fileId: int) -> dict:
        '''Cached Test_Info lookup. testTup is (test_num, test_name).'''
        key = (fileId, testTup[0], testTup[1])
        if key in self.test_info_cache:
            return self.test_info_cache[key]

        self.cursor.execute(
            "SELECT * FROM Test_Info WHERE Fid=? AND TEST_NUM=? AND TEST_NAME=?",
            [fileId, testTup[0], testTup[1]],
        )
        ...
        if val is None:
            ...
            info = dict(zip(col, val))
            info["LLimit"] = np.nan if info["LLimit"] is None else info["LLimit"]
            info["HLimit"] = np.nan if info["HLimit"] is None else info["HLimit"]
            info["LSpec"] = np.nan if info["LSpec"] is None else info["LSpec"]
            info["HSpec"] = np.nan if info["HSpec"] is None else info["HSpec"]

        self.test_info_cache[key] = info
        return info

    def _get_test_data_cache(self, testID: int, recHeader: int, fileId: int) -> dict:
        '''Build and cache full-length test data arrays for (test_id, fid).'''
        key = (testID, fileId)
        if key in self.test_data_cache:
            return self.test_data_cache[key]

        n = len(self.full_dut_cache.get(fileId, ()))
        data = np.full(n, np.nan, dtype=np.float32)
        flags = np.full(n, -1, dtype=np.int16)
        states = None
        valid_idx = []

        if recHeader == REC.PTR:
            for dut_index, result, flag in self.cursor.execute(
            ...
            info = self._get_test_info_from_id(testID, fileId)
            rslt_cnt = info.get("RSLT_PGM_CNT") if info else None
            rows = []
            for dut_index, rslts_hex, stats_hex, flag in self.cursor.execute(
                "SELECT DUTIndex, RTN_RSLT, RTN_STAT, TEST_FLAG FROM MPR_Data WHERE TEST_ID=? ORDER BY DUTIndex",
                (testID,),
            ):
                rows.append((int(dut_index), rslts_hex, stats_hex, flag))

            if rows:
                if rslt_cnt is None or rslt_cnt <= 0:
                    rslt_cnt = len(np.frombuffer(bytearray.fromhex(rows[0][1]), dtype=np.float32))
                if rslt_cnt > 0:
                    data = np.full((n, rslt_cnt), np.nan, dtype=np.float32)
                    states = np.full((n, rslt_cnt), 0xF, dtype=np.uint8)
                else:
                    data = np.array([])
                    states = np.array([])

                for dut_index, rslts_hex, stats_hex, flag in rows:
                    pos = int(dut_index) - 1
                    if 0 <= pos < n:
                        flags[pos] = flag
                        valid_idx.append(pos)
                        if rslt_cnt > 0:
                            data[pos] = np.frombuffer(bytearray.fromhex(rslts_hex), dtype=np.float32)
                            states[pos] = np.frombuffer(bytearray.fromhex(stats_hex), dtype=np.uint8)
            else:
                if rslt_cnt is None or rslt_cnt <= 0:
                    data = np.array([])
                    states = np.array([])
            ...
            # FTR
            for dut_index, flag in self.cursor.execute(
        ...
        entry = {
            ...
            "flags": flags,
            "states": states,
            "valid_test_idx": np.asarray(sorted(valid_idx), dtype=np.intp),
        }
        self.test_data_cache[key] = entry
        return entry

    def _get_test_info_from_id(self, testID: int, fileId: int) -> dict:
        '''Find Test_Info by TEST_ID from the cached info map (helper for MPR row count).'''
        for key, info in self.test_info_cache.items():
            if key[0] == fileId and info.get("TEST_ID") == testID:
                return info
        # Lazy fallback
        self.cursor.execute(
            "SELECT * FROM Test_Info WHERE Fid=? AND TEST_ID=?",
            (fileId, testID),
        )
        col = [tup[0] for tup in self.cursor.description]
        val = self.cursor.fetchone()
    ...
    def _head_site_row_indices(self, heads: list[int], sites: list[int], fileId: int) -> np.ndarray:
        '''Combine cached head/site row-index lists into one sorted unique array.'''
        if not heads or not sites:
            return np.empty(0, dtype=np.intp)

        parts = []
        if -1 in sites:
            for head in heads:
                key = (fileId, int(head), None)
                if key in self.head_site_cache:
                    parts.append(self.head_site_cache[key])
        else:
            for head in heads:
                for site in sites:
                    key = (fileId, int(head), int(site))
                    if key in self.head_site_cache:
                        parts.append(self.head_site_cache[key])
    ...
    def getTestDataFromHeadSite(self, testTup: tuple, heads: list[int], sites: list[int], fileId: int) -> dict:
        ...
        info = self._get_test_info(testTup, fileId)
        if not info:
            return {}

        testID = info["TEST_ID"]
        recHeader = info["recHeader"]
        head_site_idx = self._head_site_row_indices(heads, sites, fileId)
        cache_entry = self._get_test_data_cache(testID, recHeader, fileId)
        valid_idx = np.intersect1d(head_site_idx, cache_entry["valid_test_idx"], assume_unique=True)
        full_dut = self.full_dut_cache[fileId]

        if recHeader == REC.PTR:
            return {
                "dutList": full_dut[valid_idx],
                "dataList": cache_entry["data"][valid_idx],
                "flagList": cache_entry["flags"][valid_idx].astype(np.uint8),
            }
        ...
        if recHeader == REC.MPR:
            ...
            return {
                "dutList": full_dut[valid_idx],
                "dataList": cache_entry["data"][valid_idx].T,
                "stateList": cache_entry["states"][valid_idx].T,
                "flagList": cache_entry["flags"][valid_idx].astype(np.uint8),
            }
    ...
    def getTestDataFromDutIndex(self, testTup: tuple, duts: list[int], fileId: int) -> dict:
        ...
        flagList = np.full(dut_count, -1, dtype=np.int16)

        if recHeader == REC.PTR:
            dataList = np.full(dut_count, np.nan, dtype=np.float32)
            for i, dut in enumerate(dutList):
                pos = int(dut) - 1
                if 0 <= pos < n:
                    dataList[i] = cache_entry["data"][pos]
                    flagList[i] = cache_entry["flags"][pos]
            return {"dutList": dutList,
                    "dataList": dataList,
                    "flagList": flagList}

        if recHeader == REC.MPR:
            if cache_entry["data"].ndim == 2:
                rslt_cnt = cache_entry["data"].shape[1]
                dataList = np.full((dut_count, rslt_cnt), np.nan, dtype=np.float32)
                stateList = np.full((dut_count, rslt_cnt), 0xF, dtype=np.uint8)
                for i, dut in enumerate(dutList):
                    pos = int(dut) - 1
                    if 0 <= pos < n:
                        dataList[i] = cache_entry["data"][pos]
                        stateList[i] = cache_entry["states"][pos]
                        flagList[i] = cache_entry["flags"][pos]
                return {"dutList": dutList,
                        "dataList": dataList.T,
