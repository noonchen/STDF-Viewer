        ...
        for fid in sorted(testInfo.keys()):
            ...
            for site, data_per_site in sitesData.items():
                ...
                pitem.addItem(pg.PlotDataItem(x=x, y=y, pen=None,
                                              ...
                                              symbolSize=8, symbolBrush=color,
                                              name=f"Site {site}"))
                # mean
                mean = data_per_site["Mean"]
                if True and ~np.isnan(mean):
